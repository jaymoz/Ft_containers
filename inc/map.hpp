...
namespace ft
{
	...
	class Map
	{
		public:
			...
			typedef Btree<key_type, mapped_type>*                   node;
		...
		private:
			...
			node            _root;
			size_type       _length;
			void ft_check_tree(node n)
			{
				if (!n)
					return;
				ft_check_tree(n->left);
				if (n->parent && !n->end)
					std::cout << n->pair.first << "=" << n->pair.second << std::endl;
				ft_check_tree(n->right);
			};
			node ft_new_node(key_type key, mapped_type value, node parent, bool end = false)
			{
				node new_node = new Btree<key_type, mapped_type>();
				new_node->pair = std::make_pair(key, value);
				new_node->right = 0;
				new_node->left = 0;
			...
			{
				_root = ft_new_node(key_type(), mapped_type(), 0);
				_root->right  = ft_new_node(key_type(), mapped_type(), _root, true);
				_length = 0;
			};
		...
		public:
			...
			{
				return (std::numeric_limits<size_type>::max() / (sizeof(ft::Btree<key_type, mapped_type>)));
			};
			...
			std::pair<iterator, bool> insert(const value_type &value)
			{
				...
				++_length;
				return (std::make_pair(iterator(ft_insert_node(_root, value.first, value.second)), true));
			};
			iterator insert(iterator position, const value_type &value)
			{
				...
				if ((tmp = find(value.first)) != end())
					return (tmp);
				++_length;
				return (iterator(ft_insert_node(position.node(), value.first, value.second)));
			};
			template <class InputIterator>
	...
	bool operator==(const Map<Key, T, Compare, Alloc> &left, const Map<Key, T, Compare, Alloc> &right)
	{
		if (left.size() != right.size())
			return (false);
		typename ft::Map<Key, T, Compare, Alloc>::const_iterator it = right.begin();
		typename ft::Map<Key, T, Compare, Alloc>::const_iterator it2 = left.begin();
		while (it != right.end())
		{
			if (*it != *it2)
				return (false);
	...
	bool operator>(const Map<Key, T, Compare, Alloc> &left, const Map<Key, T, Compare, Alloc> &right)
	{
		if (left.size() > right.size())
			return (true);
		typename ft::Map<Key, T, Compare, Alloc>::const_iterator it = left.begin();
		typename ft::Map<Key, T, Compare, Alloc>::const_iterator it2 = right.begin();
