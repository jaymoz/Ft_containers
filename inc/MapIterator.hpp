...
namespace ft
{
	...
	{
		public:
			typedef Btree<K, T>*			pointer;
			typedef std::pair<K, T>     	value_type;
			typedef std::pair<K, T>&		reference;
		protected:
			pointer _ptr;
		private:
			pointer ft_get_successor(pointer ptr)
			{
				pointer next;
				if (!ptr->right)
				{
			...
			pointer ft_get_predecessor(pointer ptr)
			{
				pointer next;
		...
		public:
			...
			{
				_ptr = ft_get_successor(_ptr);
				return (*this);
			};
			MapIterator &operator--(void)
			{
				_ptr = ft_get_predecessor(_ptr);
				return (*this);
			};
